...
\begin{frame}{panic write flag}
  ...
  \item Before v5.3 driver had no detect panic write
  \item Now drivers can check \code{oops_panic_write}
  \end{itemize}
\end{frame}
...
\begin{frame}{Raw NAND: A journey supporting Micron's hardware 2/3}
  ...
  \item [Issue 2] Hardware bug: on certain chips where on-die ECC is
    supposedly mandatory, \code{->get_features()} returns that it is
    disabled.
    \onslide<2->
  ...
  \item [Issue 3] Hardware bug: has explained above, a chip without
    ...
    the engine with a \code{->set_feature()} will work despite the fact
